...
public class Player extends Character {
	...
	public void HandleUserInput(Game g) {
		...
		// no input
		if (onGround && !userInput.Key_A_Pressed && !userInput.Key_D_Pressed && currentState != State.Looting) {
			// lastState = currentState;
			currentState = State.Idle;
		}
	...
	public void Update(Game g) {
		...
		tmp.y = 0.0f;
		tmp.x = 0.0f;
		if (g.CollisionManager.BoxCollision(this, this.Collision.box, g.CollisionBoxList, Speed, currentDirection,
				Velocity, tmp)) {
			// what happens
		}
		Velocity.x = tmp.x;
	...
	public void Draw(Graphics2D g2, Game g) {
		...
		g2.drawImage(Sprite.Texture, (int) (Position.x - g.Camera.cameraX), (int) (Position.y - g.Camera.cameraY), 150,
				100, null);
	}
	...
	public Vector<Chest> GetLootablesInRange(Game g, int range) {
		...
		for (Entity e : g.EntityList) {
			...
			if (e.EntityType != EntityType.Chest) {
				continue;
			}
